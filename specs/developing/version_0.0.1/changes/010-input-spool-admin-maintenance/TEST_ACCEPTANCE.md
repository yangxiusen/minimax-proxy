...
| FT-006A | Manager 查看/下载媒体输入 | 存在本地托管图片或音频输入 | 在详情弹窗媒体输入行点击“查看”和“下载” | 查看新标签打开原文件；下载返回附件；均不暴露本地路径 | 待执行 |
...
| `GET /manager/api/tasks/{task_id}/inputs/{input_id}/content` | inline 查看 | Manager Cookie | 200，Content-Disposition 为 inline，返回原始文件字节 | 待执行 |
| `GET /manager/api/tasks/{task_id}/inputs/{input_id}/content?download=1` | 下载 | Manager Cookie | 200，Content-Disposition 为 attachment，返回原始文件字节 | 待执行 |
| `GET /manager/api/tasks/{task_id}/inputs/{input_id}/content` | 未登录 | 无 Cookie | 401 | 待执行 |
...
| 鉴权 | 未登录不能访问 Manager 输入文件内容 | 单元测试 | 待执行 |
...
| Manager | `GET /manager/api/tasks/{task_id}/inputs/{input_id}/content` | 媒体输入“查看/下载”、不同浏览器预览兼容性 | 变更 issue 或测试记录 |
