DROP TABLE IF EXISTS video_tasks_v2;

CREATE TABLE video_tasks_v2 (
...
SELECT
    ...
    request_hash, status, cancel_locked, upstream_id, gradio_event_id,
    gallery_before_json, result_internal_url, result_public_url,
    CASE WHEN resolution = '768P' THEN '480P' ELSE resolution END,
    duration, ratio_requested, ratio_actual, usage_total_seconds,
    usage_input_seconds, usage_output_seconds, usage_input_image_count,
    error_code, error_message, created_at, updated_at, started_at, finished_at,
    expires_at, deleted_at, version
FROM video_tasks;

DROP TABLE video_tasks;
ALTER TABLE video_tasks_v2 RENAME TO video_tasks;
...
CREATE UNIQUE INDEX uq_video_tasks_active_upstream ON video_tasks(upstream_id) WHERE status IN ('dispatching','running','reconciling');
